...
def test_analysis_data_table(page: Page) -> None:
    ...
    # テーブルまたはデータコンテナの存在確認
    data_container = page.locator(
        ".table, .analysis-data, .user-data, [data-testid='analysis-table']"
    )
...
def test_analysis_month_navigation(page: Page) -> None:
    ...
    prev_buttons = page.locator(
        "a[href*='month='], button[data-month], .btn-prev, .month-prev"
    )
...
def test_analysis_group_data_display(page: Page) -> None:
    ...
    if page_content:
        ...
        has_group_info = any(
            indicator in page_content for indicator in group_indicators
        )
