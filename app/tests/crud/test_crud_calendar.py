...
def db_with_calendar_test_data(db: Session) -> Session:
    ...
    first_day_next_month = (
        first_day_this_month.replace(
            year=today.year + (today.month // 12), month=(today.month % 12) + 1
        )
        if today.month < 12
        else first_day_this_month.replace(year=today.year + 1, month=1)
    )
    ...
    for dt in unique_dates:
        attendance_in = AttendanceCreate(
            user_id=str(user.id), date=dt, location_id=int(location.id)
        )
...
def test_get_month_attendances(db_with_calendar_test_data: Session) -> None:
    ...
    attendances = calendar_crud.get_month_attendances(
        db=db, first_day=first_day, last_day=last_day
    )
    # 今月作成したデータ数 (1日、2日、今日（今日が1日or2日の場合は重複しない）)
    expected_count = 0
    if first_day <= first_day <= last_day:
        expected_count += 1
    if first_day <= first_day + timedelta(days=1) <= last_day:
        expected_count += 1
    if first_day <= today <= last_day and today.day > 2:
        expected_count += 1  # 今日が1,2日と重複しない場合のみカウント
...
def test_count_day_attendances(db_with_calendar_test_data: Session) -> None:
    ...
    existing = (
        ...
        .filter(AttendanceModel.date == target_date_no_data)
        .all()
    )
    ...
    count_no_data = calendar_crud.count_day_attendances(
        db=db, target_date=target_date_no_data
    )
...
def test_get_month_attendance_counts(db_with_calendar_test_data: Session) -> None:
    ...
    counts = calendar_crud.get_month_attendance_counts(
        db=db, first_day=first_day, last_day=last_day
    )
    ...
    if first_day.day <= last_day.day:  # 月初が月末より前（通常）
        if 1 >= first_day.day and 1 <= last_day.day:
            expected_days_with_data += 1
        if 2 >= first_day.day and 2 <= last_day.day:
            expected_days_with_data += 1
        if today.day > 2 and today.day >= first_day.day and today.day <= last_day.day:
            expected_days_with_data += 1
