"""
勤怠APIエンドポイント (/api/v1/attendances) のテスト
"""
...
async def test_create_attendance_success(async_client: AsyncClient) -> None:
    ...
    user_id = await create_test_user_via_api(
        async_client, "att_user_success", "Att User Success", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation Success"
    )
    ...
    response = await async_client.post("/api/v1/attendances", data=payload)

    assert response.status_code == status.HTTP_204_NO_CONTENT

    # 削除したエンドポイントを使わずに、get_attendancesを使ってデータを確認
    get_all_response = await async_client.get("/api/v1/attendances")
...
async def test_create_attendance_duplicate(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeDup"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_dup", "Att User Dup", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation Dup"
    )

    test_date = (date.today() - timedelta(days=1)).isoformat()  # 昨日日付を使用

    payload = {"user_id": user_id, "date": test_date, "location_id": str(location_id)}

    # 1回目の作成 (成功するはず)
    response1 = await async_client.post("/api/v1/attendances", data=payload)
    assert response1.status_code == status.HTTP_204_NO_CONTENT

    # 2回目の作成 (失敗するはず)
    response2 = await async_client.post("/api/v1/attendances", data=payload)
...
async def test_create_attendance_invalid_user(async_client: AsyncClient) -> None:
    ...
    """
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation InvUser"
    )
    test_date = date.today().isoformat()
    ...
    assert (
        "User with id non_existent_user not found" in response.json()["detail"]
    )  # エラーメッセージを確認
...
async def test_create_attendance_invalid_location(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeInvLoc"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_inv_loc", "Att User InvLoc", group_id, user_type_id
    )
    test_date = date.today().isoformat()
    ...
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert (
        f"Location with id {non_existent_location_id} not found"
...
async def test_create_attendance_invalid_date_format(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeInvDate"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_inv_date", "Att User InvDate", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation InvDate"
    )
...
# --- GET /api/v1/attendances/day/{day} Tests ---
...
async def test_get_day_attendance_success(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeDay"
    )
    user1_id = await create_test_user_via_api(
        async_client, "att_user_day1", "Att User Day 1", group_id, user_type_id
    )
    user2_id = await create_test_user_via_api(
        async_client, "att_user_day2", "Att User Day 2", group_id, user_type_id
    )
    location1_id = await create_test_location_via_api(
        async_client, "AttTestLocation Day1"
    )
    location2_id = await create_test_location_via_api(
        async_client, "AttTestLocation Day2"
    )
    ...
    await async_client.post(
        "/api/v1/attendances",
        data={"user_id": user1_id, "date": test_day, "location_id": str(location1_id)},
    )
    await async_client.post(
        "/api/v1/attendances",
        data={"user_id": user2_id, "date": test_day, "location_id": str(location2_id)},
    )
    await async_client.post(
        "/api/v1/attendances",
        data={"user_id": user1_id, "date": other_day, "location_id": str(location1_id)},
    )

    response = await async_client.get(f"/api/v1/attendances/day/{test_day}")
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    ...
    all_entries = [
        entry for sublist in day_data_values for entry in sublist
    ]  # 全エントリをフラット化
...
# --- PUT /api/v1/attendances/{attendance_id} Tests ---
...
async def test_update_attendance_success(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypePut"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_put", "Att User Put", group_id, user_type_id
    )
    location1_id = await create_test_location_via_api(
        async_client, "AttTestLocation Put1"
    )
    location2_id = await create_test_location_via_api(
        async_client, "AttTestLocation Put2"
    )
    ...
    create_payload = {
        "user_id": user_id,
        "date": test_date,
        "location_id": str(location1_id),
    }
    create_response = await async_client.post(
        "/api/v1/attendances", data=create_payload
    )
    ...
    response = await async_client.put(
        f"/api/v1/attendances/{attendance_id}", data=update_payload
    )

    assert response.status_code == status.HTTP_204_NO_CONTENT

    # 更新後のデータを確認
    get_updated_response = await async_client.get("/api/v1/attendances")
    ...
    assert (
        updated_attendance["location_id"] == location2_id
    )  # 更新後の location_id を確認
...
async def test_update_attendance_not_found(async_client: AsyncClient) -> None:
    ...
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation PutNF"
    )
    update_payload = {"location_id": str(location_id)}
    response = await async_client.put(
        f"/api/v1/attendances/{non_existent_attendance_id}", data=update_payload
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert (
        f"Attendance with id {non_existent_attendance_id} not found"
        in response.json()["detail"]
    )
...
async def test_update_attendance_invalid_location(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypePutInvLoc"
    )
    user_id = await create_test_user_via_api(
        async_client,
        "att_user_put_invloc",
    ...
    location1_id = await create_test_location_via_api(
        async_client, "AttTestLocation PutInvLoc1"
    )
...
# --- DELETE /api/v1/attendances/{attendance_id} Tests ---
...
async def test_delete_attendance_success(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeDel"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_del", "Att User Del", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation Del"
    )
    ...
    # 削除リクエスト
    response = await async_client.delete(f"/api/v1/attendances/{attendance_id}")
...
async def test_delete_attendance_not_found(async_client: AsyncClient) -> None:
    ...
    response = await async_client.delete(
        f"/api/v1/attendances/{non_existent_attendance_id}"
    )
    assert (
        response.status_code == status.HTTP_404_NOT_FOUND
    )  # ルーター側のget_or_404が先に機能する
    assert (
        f"Attendance with id {non_existent_attendance_id} not found"
        in response.json()["detail"]
    )
...
# --- DELETE /api/v1/attendances?user_id={user_id}&date={date} Tests ---


async def test_delete_attendance_by_user_date_success(
    async_client: AsyncClient,
) -> None:
    """ユーザーIDと日付指定で勤怠データを正常に削除できることをテストします。"""
    group_id = await create_test_group_via_api(async_client, "AttTestGroupDelUD")
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeDelUD"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_del_ud", "Att User Del UD", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation DelUD"
    )
    ...
    # 削除リクエスト
    response = await async_client.delete(
        f"/api/v1/attendances?user_id={user_id}&date={test_date}"
    )
...
async def test_delete_attendance_by_user_date_user_not_found(
    async_client: AsyncClient,
) -> None:
    ...
    test_date = date.today().isoformat()
    response = await async_client.delete(
        f"/api/v1/attendances?user_id={non_existent_user_id}&date={test_date}"
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
async def test_delete_attendance_by_user_date_record_not_found(
    async_client: AsyncClient,
) -> None:
    """削除対象のレコードが存在しない場合でも404エラーが発生することを確認します。"""
    group_id = await create_test_group_via_api(async_client, "AttTestGroupDelUDNF")
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeDelUDNF"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_del_ud_nf", "Att User Del UD NF", group_id, user_type_id
    )
...
async def test_delete_attendance_by_user_date_invalid_date(
    async_client: AsyncClient,
) -> None:
    """無効な日付形式でDELETEすると422エラーが発生することをテストします。"""
    group_id = await create_test_group_via_api(async_client, "AttTestGroupDelUDID")
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeDelUDID"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_del_ud_id", "Att User Del UD ID", group_id, user_type_id
    )
    invalid_date = "invalid-date-format"

    response = await async_client.delete(
        f"/api/v1/attendances?user_id={user_id}&date={invalid_date}"
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    # ステータスコードの確認のみで十分
