...
# API エンドポイントのベースパス
API_ENDPOINT = "/api/v1/user_types"
...
async def test_create_user_type_missing_name(async_client: AsyncClient) -> None:
    """
    POST /api/v1/user_types - 社員種別名がない場合に 400/422 エラーが返されることをテストします。
    """
...
async def test_update_user_type_not_found(async_client: AsyncClient) -> None:
    ...
    response = await async_client.put(
        f"{API_ENDPOINT}/{non_existent_user_type_id}", json=payload
    )
...
async def test_update_user_type_duplicate_name(
    ...
    user_type1 = crud_user_type.create(
        db, obj_in=UserTypeCreate(name="UserType To Update")
    )
    user_type2 = crud_user_type.create(
        db, obj_in=UserTypeCreate(name="Existing Other UserType")
    )
    db.commit()
    ...
    response = await async_client.put(
        f"{API_ENDPOINT}/{user_type_id_to_update}", json=payload
    )
...
async def test_delete_user_type_success(async_client: AsyncClient, db: Session) -> None:
    ...
    user_type_to_delete = crud_user_type.create(
        db, obj_in=UserTypeCreate(name="UserType To Delete")
    )
    db.commit()
