...
async def setup_basic_data(
    ...
    try:
        ...
        # データ削除 (作成されたものだけ削除)
        if (
            created_data.get("location")
            and created_data["location"].get("id") is not None
        ):
            try:
                await async_client.delete(
                    f"/api/v1/locations/{created_data['location']['id']}"
                )
            except Exception as e:
                print(f"Error deleting location {created_data['location']['id']}: {e}")
        if (
            created_data.get("user_type")
            and created_data["user_type"].get("id") is not None
        ):
            try:
                await async_client.delete(
                    f"/api/v1/user_types/{created_data['user_type']['id']}"
                )
            except Exception as e:
                print(
                    f"Error deleting user_type {created_data['user_type']['id']}: {e}"
                )
        if created_data.get("group") and created_data["group"].get("id") is not None:
            try:
                await async_client.delete(
                    f"/api/v1/groups/{created_data['group']['id']}"
                )
            except Exception as e:
...
async def setup_test_users(
    ...
    try:
        ...
        response = await async_client.post(
            "/api/v1/groups", json={"name": group_a_name}
        )
        response.raise_for_status()
        ...
        response = await async_client.post(
            "/api/v1/groups", json={"name": group_b_name}
        )
        response.raise_for_status()
        ...
        response = await async_client.post(
            "/api/v1/user_types", json={"name": type_x_name}
        )
        response.raise_for_status()
        ...
        response = await async_client.post(
            "/api/v1/user_types", json={"name": type_y_name}
        )
        response.raise_for_status()
        ...
        response = await async_client.post(
            "/api/v1/locations", json={"name": location_name}
        )
        response.raise_for_status()
        ...
        user_data_to_create = [
            {
                ...
                "username": "User AX",
                "group_id": group_a["id"],
                "user_type_id": type_x["id"],
            },
            {
                "id": "user_ay",
                "username": "User AY",
                "group_id": group_a["id"],
                "user_type_id": type_y["id"],
            },
            {
                "id": "user_bx",
                "username": "User BX",
        ...
        for user_payload in user_data_to_create:
            ...
            user_info = {
                **created_user,
                ...
                if created_user["group_id"] == group_a["id"]
                else group_b_name,
                "user_type_name": type_x_name
                if created_user["user_type_id"] == type_x["id"]
                else type_y_name,
            }
        ...
        await async_client.post(
            ...
            data={
                "user_id": "user_ax",
                "date": test_date,
                "location_id": str(created_data["location"]["id"]),
            },
        )
...
async def test_download_csv_success_all(
    ...
    first_user_info = next(
        (u for u in setup_test_users["users"] if u["id"] == expected_user_id_order[0]),
        None,
    )
    assert (
        first_user_info is not None
    ), "Fixture did not provide expected first user info"

    response = await async_client.get("/api/v1/csv/download")
    assert response.status_code == status.HTTP_200_OK
    assert response.headers["content-type"] == "text/csv; charset=utf-8"
    assert (
        'attachment; filename="work_entries.csv"'
    ...
    rows = list(reader)
    assert (
        len(rows) >= len(expected_user_id_order) + 1
    )  # ヘッダー + 作成したユーザー数以上
    ...
    fixture_user_ids_in_csv = [
        row[1] for row in data_rows if row[1] in expected_user_id_order
    ]

    assert (
        fixture_user_ids_in_csv == expected_user_id_order
    ), f"CSV user ID order mismatch. Expected: {expected_user_id_order}, Got: {fixture_user_ids_in_csv}"

    # (オプション) 個々のユーザーデータの簡単な検証 (例: 最初のユーザー)
    first_data_row = next(
        (row for row in data_rows if row[1] == first_user_info["id"]), None
    )
...
async def test_download_csv_success_month_sjis(
    ...
    try:
        ...
        if response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY:
            ...
            if (
                all_users_resp.status_code == status.HTTP_200_OK
                and all_users_resp.json().get("users")
            ):
        ...
        await async_client.post(
            ...
            data={
                ...
                "date": test_date_in_month,
                "location_id": str(location["id"]),
            },
        )

        response = await async_client.get(
            f"/api/v1/csv/download?month={month_param}&encoding=sjis"
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.headers["content-type"] == "text/csv; charset=shift_jis"
        assert (
            f'attachment; filename="work_entries_{month_param}.csv"'
            in response.headers["content-disposition"]
        )
        ...
        try:
            ...
            pytest.fail(
                f"Date header {target_month.strftime('%Y/%m/%d')} not found in CSV"
            )
