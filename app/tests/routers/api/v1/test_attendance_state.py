...
async def test_refresh_attendance_preserves_month(async_client: AsyncClient) -> None:
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeMonth"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_month", "Att User Month", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation Month"
    )
...
async def test_refresh_user_attendance_preserves_user_view(
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeUser"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_view", "Att User View", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation User"
    )
...
async def test_refresh_weekly_attendance_preserves_selected_week(
    async_client: AsyncClient,
) -> None:
    """
    ...
    user_type_id = await create_test_user_type_via_api(
        async_client, "AttTestUserTypeWeek"
    )
    user_id = await create_test_user_via_api(
        async_client, "att_user_week", "Att User Week", group_id, user_type_id
    )
    location_id = await create_test_location_via_api(
        async_client, "AttTestLocation Week"
    )
