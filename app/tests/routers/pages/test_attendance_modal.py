...
async def test_attendance_modal_handles_nullable_location_order(
    async_client, db
) -> None:
    group = crud_group.create(db, obj_in=GroupCreate(name="nullable-order-group"))
    user_type = crud_user_type.create(
        db, obj_in=UserTypeCreate(name="nullable-order-type")
    )
    for name, order in (
        ("Modal Zero", 0),
        ("Modal Later", 20),
        ("Modal Unordered", None),
    ):
        crud_location.create(
            db,
            obj_in=LocationCreate(name=name, category="Office", order=order),
        )
    crud_user.create(
        db,
        obj_in=UserCreate(
            id="nullable-order-user",
            username="Nullable Order User",
            group_id=int(group.id),
            user_type_id=int(user_type.id),
        ),
    )
    ...
    response = await async_client.get(
        "/attendance/modals/nullable-order-user/2031-05-03"
    )
