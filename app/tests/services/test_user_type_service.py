...
def test_validate_user_type_update_success(db: Session) -> None:
    ...
    user_type_service.validate_user_type_update(
        db,
        user_type_id_to_update=int(user_type_to_update.id),
        user_type_in=update_in_new_name,
    )

    # ケース2: 自分自身の名前に更新（変更なし）
    update_in_same_name = schemas.user_type.UserTypeUpdate(
        name=str(user_type_to_update.name)
    )
    user_type_service.validate_user_type_update(
        db,
        user_type_id_to_update=int(user_type_to_update.id),
        user_type_in=update_in_same_name,
    )
...
def test_create_user_type_with_validation_success(db: Session) -> None:
    ...
    # DBでも確認
    db_user_type = (
        db.query(models.UserType).filter(models.UserType.name == user_type_name).first()
    )
    assert db_user_type
...
def test_create_user_type_with_validation_fail(db: Session) -> None:
    ...
    with pytest.raises(HTTPException):
        user_type_service.create_user_type_with_validation(
            db, user_type_in=user_type_in
        )
...
def test_update_user_type_with_validation_fail_duplicate(db: Session) -> None:
    ...
    user_type_in_update = schemas.user_type.UserTypeUpdate(
        name=str(other_user_type.name)
    )
    with pytest.raises(HTTPException):
        user_type_service.update_user_type_with_validation(
            db,
            user_type_id=int(user_type_to_update.id),
            user_type_in=user_type_in_update,
        )
