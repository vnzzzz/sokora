...
def test_get_user_by_username(db_with_data: Session) -> None:
    ...
    user_type = (
        db.query(models.UserType).filter(models.UserType.name == "Test Type").first()
    )
    # None チェックを先に実行
...
def test_validate_dependencies_success(db_with_data: Session) -> None:
    ...
    user_service.validate_dependencies(
        db, group_id=int(group.id), user_type_id=int(user_type.id)
    )
...
def test_validate_dependencies_fail(db_with_data: Session) -> None:
    ...
    with pytest.raises(HTTPException) as excinfo_group:
        user_service.validate_dependencies(
            db, group_id=9999, user_type_id=int(user_type.id)
        )
    ...
    with pytest.raises(HTTPException) as excinfo_type:
        user_service.validate_dependencies(
            db, group_id=int(group.id), user_type_id=9999
        )
...
def test_update_user_with_validation_success(db_with_data: Session) -> None:
    ...
    user_to_update_in = schemas.UserCreate(
        id="user_to_update",
        username="User To Update Name",
        group_id=int(group.id),
        user_type_id=int(user_type.id),
    )
    user_to_update = crud.user.create(db=db, obj_in=user_to_update_in)

    new_group = crud.group.create(
        db, obj_in=schemas.GroupCreate(name=random_lower_string())
    )
    new_user_type = crud.user_type.create(
        db, obj_in=UserTypeCreate(name=random_lower_string())
    )
    db.commit()
...
def test_update_user_with_validation_fail_duplicate_username(
    ...
    with pytest.raises(HTTPException) as excinfo:
        user_service.update_user_with_validation(
            db, user_id=str(user2.id), user_in=user_in_update
        )
    assert excinfo.value.status_code == 400
    assert "ユーザー名" in excinfo.value.detail
