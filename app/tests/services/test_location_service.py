...
def test_validate_location_update_success(db: Session) -> None:
    ...
    location_service.validate_location_update(
        db,
        location_id_to_update=int(location_to_update.id),
        location_in=update_in_new_name,
    )

    # ケース2: 自分自身の名前に更新（変更なし）
    update_in_same_name = schemas.location.LocationUpdate(
        name=str(location_to_update.name)
    )
    location_service.validate_location_update(
        db,
        location_id_to_update=int(location_to_update.id),
        location_in=update_in_same_name,
    )
...
def test_create_location_with_validation_success(db: Session) -> None:
    ...
    # DBでも確認
    db_location = (
        db.query(models.Location).filter(models.Location.name == location_name).first()
    )
    assert db_location
