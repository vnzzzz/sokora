...
def user_page(request: Request, db: Session = Depends(get_db)) -> Any:
    ...
    sorted_group_names = sorted(
        grouped_users.keys(), key=lambda g: group_orders.get(g, float("inf"))
    )
...
async def user_modal(
    request: Request, user_id: Optional[str] = None, db: Session = Depends(get_db)
) -> Any:
...
def handle_create_user_row(
    ...
    try:
        ...
        if not user_row_data:  # 基本的に作成直後なので存在するはずだが念のため
            raise HTTPException(
                status_code=500, detail="Failed to retrieve created user details"
            )
...
def handle_update_user_row(
    ...
    try:
        ...
        if not user_row_data:  # 基本的に更新直後なので存在するはずだが念のため
            raise HTTPException(
                status_code=500, detail="Failed to retrieve updated user details"
            )
