...
def get_calendar(
    ...
    if (
        cached_data
        and current_time - _calendar_cache_timestamp.get(month, 0) < _calendar_cache_ttl
    ):
        ...
        try:
            ...
            location_objects = sorted(
                location_objects_unsorted, key=lambda loc: int(loc.id)
            )
            ...
            location_color_map: Dict[int, Dict[str, str]] = {
                int(loc.id): get_location_color_classes(int(loc.id))
            ...
            for loc_data in calendar_data.get("locations", []):
                ...
                matched_loc_obj = next(
                    (
                        loc
                        for loc in location_objects
                        if str(loc.name) == loc_data["name"]
                    ),
                    None,
                )
                if matched_loc_obj:
            ...
            logger.error(
                f"カレンダーデータ取得中にエラー ({month}): {e}", exc_info=True
            )
            calendar_data = None  # エラー発生
    ...
    return templates.TemplateResponse(
        "components/top/summary_calendar.html", context, headers=headers
    )


@router.get("/day/{day}", response_class=HTMLResponse)
def get_day_detail(request: Request, day: str, db: Session = Depends(get_db)) -> Any:
    """指定された日の詳細を表示します
    ...
    # グループのorder情報を保存する辞書
    group_orders = {
        g.id: (g.order if g.order is not None else float("inf")) for g in groups
    }
    ...
    sorted_groups = sorted(
        groups, key=lambda g: (group_orders.get(g.id, float("inf")), g.id or 9999)
    )
    ...
    for location_name, users_list in attendance_data.items():
        ...
        matched_loc = next(
            (loc for loc in location_objects if str(loc.name) == location_name), None
        )
        ...
        for user_data in users_list:
            ...
            user_data["location_text_class"] = (
                location_text_class  # テキストクラスを追加
            )
            ...
            if group_name not in organized_by_group:
                organized_by_group[group_name] = {
                    ...
                    "group_id": int(user_obj.group_id)
                    if user_obj.group_id is not None
                    else 9999,
                    "group_order": group_orders.get(
                        user_obj.group_id, float("inf")
                    ),  # groupのorder情報を追加
                }
            ...
            organized_by_group[group_name]["user_types_data"][user_type_name].append(
                user_data
            )
    ...
    def get_group_sort_key(item: tuple[str, Dict[str, Any]]) -> tuple:
        ...
        if isinstance(group_data, dict):
            ...
            return (
                group_data.get("group_order", float("inf")),
                group_data.get("group_id", 9999),
            )
        logger.warning(
            f"Unexpected type for group data: {type(group_data)} in item: {item}"
        )
        return (float("inf"), 9999)  # Default sort value for unexpected types

    # 最終的なグループ主キーの辞書を、グループのorderに基づいてソートします。
    sorted_organized_by_group = dict(
        sorted(
    ...
    has_data = bool(attendance_data) and any(
        len(users) > 0 for users in attendance_data.values()
    )
    ...
    return templates.TemplateResponse("components/top/day_detail.html", context)
