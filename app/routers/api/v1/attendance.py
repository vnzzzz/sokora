...
async def create_attendance(
    ...
    try:
        ...
        attendance_in = AttendanceCreate(
            user_id=user_id, date=attendance_date, location_id=location_id, note=note
        )
...
async def update_attendance(
    ...
    try:
        ...
        # 更新処理
        updated_obj = attendance.update(
            db=db, db_obj=attendance_obj, obj_in=attendance_in
        )  # 更新後のオブジェクト取得
...
async def delete_attendance(
    ...
    try:
        ...
        current_week = extract_week_from_request(
            request, cast(date, attendance_obj.date)
        )
...
def delete_attendance_by_user_date(
    ...
    try:
        ...
        logger.debug(
            f"ユーザー '{user_id}' の日付 '{date_str}' の勤怠削除に成功しました"
        )
