...
この文書は、FastAPI JSON APIのpublic HTTP contractと、JSON API / page-HTMX adapterの責務境界をSSoTとする。data modelは [DB requirements](../db/requirements.md)、UI behaviorは [UI requirements](../ui/requirements.md) を参照する。
...
認証方式、cookie、OIDC discovery、local admin fallbackのarchitectureは [ADR 0002](../adr/0002-authentication-runtime.md)、runtime設定contractは [Production runtime](../deployment/runtime.md) を参照する。この文書ではOIDC library内部処理やcookie implementationを重複して保守しない。
...
- APIが扱うfield/type/constraintは [DB requirements](../db/requirements.md) に従う。
