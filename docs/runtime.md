...
- `/auth/settings`はlocal adminだけが利用できる。OIDC設定の保存・明示無効化・連携解除・standard discovery接続確認を提供する。OIDCをenabledで保存する場合はdiscovery取得とmetadata issuer一致を必須検証し、失敗時は有効設定をDBへ保存しない。保存済みclient secretは画面へ再表示しない。
- application logoutはprovider logoutより優先する。shared DB unavailable等でOIDC設定を解決できない場合はprovider logoutを省略し、application session破棄を継続する。
...
- multi-replicaではshared PostgreSQLの`auth_config`をrequest時に参照し、process-local OIDC settings cacheを共有stateとして持たない。
