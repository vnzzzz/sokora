...
sokoraのproduction artifactは、deployment targetに依存しない共通OCI imageとする。この文書は**image/runtimeが満たすprovider非依存contract**のSSoTであり、providerごとの実装statusと入口は [Deployment guide](README.md) を参照する。共通imageとdeployment adapterを分離する判断理由は [ADR 0004](../adr/0004-provider-neutral-oci-deployment.md) に記録する。
...
認証architectureの理由とsecurity boundaryは [ADR 0002](../adr/0002-authentication-runtime.md)、HTTP guard behaviorは [API requirements](../api/requirements.md) を参照する。
...
file-backed SQLiteのadmin backup/restoreは [SQLite database management](../operations/sqlite-database-management.md) を参照する。
...
PostgreSQLのonline migrationはadvisory lockでsokora migration process間を直列化する。schema lifecycleの詳細は [DB requirements](../db/requirements.md) を参照する。
...
現時点でclosed-network adapterは実装済み。GCP/AWS/Azure adapterは #57 / #70 / #71 でplannedであり、未実装targetをdeploy support済みとは扱わない。statusと入口は [Deployment guide](README.md) を参照する。
