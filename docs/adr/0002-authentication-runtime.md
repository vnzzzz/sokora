...
- `/auth/settings`はlocal admin専用のeditable OIDC管理画面とする。source/state表示、enable/disable、issuer/client ID/scope、secret更新、unlink、standard discovery接続確認を提供する。管理POSTはsession-backed CSRF tokenを必須とする。OIDCをenabledで保存する場合は`openid` scope、discovery取得、metadata issuerの末尾slashを含むexact一致を必須とし、失敗したcandidateを有効設定として永続化しない。
...
- DB由来OIDC設定をprocess-global mutable cacheへ保持しない。shared PostgreSQLを利用するmulti-replicaはrequestごとに同じDB stateを観測する。OIDC redirect/callback用の設定解決はshort-lived DB sessionで完結させ、IdP discovery/token/JWKS等の外部I/Oをawaitする前にDB connectionを返却する。管理画面のenabled保存もDiscovery完了後にだけshort-lived DB sessionを取得する。
